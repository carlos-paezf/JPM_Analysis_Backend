...
{
    public class ClientService : IBaseService<ClientModel, ClientEagerDTO, ClientSimpleDTO>, ISoftDeleteService
    {
        ...
        public async Task SoftDelete(string pk)
        {
            ...
            catch (Exception ex)
            {
                await _errorHandlingService.HandleExceptionAsync(
                    ex: ex, logger: _logger,
                    className: nameof(ClientService), methodName: nameof(SoftDelete));
                throw;
            }
        }
